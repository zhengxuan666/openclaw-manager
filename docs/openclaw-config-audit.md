...
| `$include`       | 官方支持                   | **已支持读取链路展开（含嵌套/边界校验）**        | 已支持               | **已完成（读取侧）**        |
| `env` & `${VAR}` | 官方支持                   | **已支持读取阶段变量替换**                       | 已支持               | **已完成（读取侧）**        |
...
## 5.1 支持 `$include` 分拆配置（已完成读取侧）

- 已实现读取链路 `$include`：支持对象节点中使用字符串或字符串数组
- 已支持嵌套 include（最大深度 10）
- 已支持 include 子文件 JSON5 解析
- 已实现合并语义：
  - 单文件 include：作为当前节点基底对象
  - 数组 include：按顺序深度合并（后者覆盖前者）
  - 同级字段：在 include 合并后再覆盖（sibling override）
- 已实现安全约束：
  - include 路径相对“声明 include 的文件”解析
  - 支持相对/绝对路径
  - 最终 canonical 路径必须在主配置目录边界内（越界报错）
  - 缺失文件 / 子文件解析失败 / 循环引用 / 深度超限均返回明确错误

当前边界（需明确）：

- 写回策略仍是 `serde_json::to_string_pretty`
- 当配置经由写回接口保存时，当前不会保留 `$include` 结构，最终会落成**展开态 JSON**
...
6. **`$include` 分拆读取与安全校验**（已完成读取侧；写回仍落成展开态）
7. **插件 allow 中空字符串清理**（待处理，潜在 schema 风险）
...
- `$include` 分拆读取与安全校验（读取侧，写回为展开态）

**下一阶段优先级**：

1. `tools` / `messages` / `commands` / `web` 结构化与可视化管理
2. hooks / cron / heartbeat / sessions 的配置管理能力补齐
